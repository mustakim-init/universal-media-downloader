...
def should_use_selenium(url: str) -> bool:
    try:
        dom = urlparse(url).netloc.lower()
    except Exception:
        return False
    domains = settings.get('selenium_domains') or []
    for entry in domains:
        e = _normalize_domain(entry)
        if not e:
            continue
        if dom == e or dom.endswith("." + e):
            return True
    # Heuristics: temporary links with huge tokens (likely HLS behind JS)
    if "m3u8" in url or "manifest.mpd" in url:
        # If the host looks like a throwaway CDN, prefer selenium headers/cookies capture
        if any(seg.isdigit() for seg in dom.split(".")) or len(dom.split(".")) >= 3:
            return True
    return False
import random
import base64
...
# ===================== Selenium/Proxy/HLS Integration (additive) =====================
# This block was added to enable Selenium-based resolution of protected/ephemeral media
# and optional ffmpeg fallback, with progress surfaced to the existing GUI queue.

import os, time, shutil, tempfile, subprocess, threading, logging
from typing import List, Dict, Optional, Tuple

# Try selenium-wire and undetected-chromedriver for better anti-bot handling
try:
    from seleniumwire import webdriver as _sw_webdriver  # pip install selenium-wire
    from selenium.webdriver.chrome.options import Options as _ChromeOptions
    _SELENIUM_WIRE_OK = True
except Exception:
    _SELENIUM_WIRE_OK = False
try:
...
# Lightweight, safe GUI notifier that won't crash if queue isn't present
def _send_gui(event_type: str, payload: dict):
    try:
        q = globals().get("gui_message_queue")
        if q is not None:
...
# Minimal cookie conversion fallback (if your CookieManager isn't available)
def _cookies_to_netscape(cookies: List[dict]) -> str:
    lines = ["# Netscape HTTP Cookie File", "# Generated by Selenium integration", ""]
    for c in cookies or []:
        try:
...
_MEDIA_PAT = re.compile(r"\.m3u8(?:\?|$)|\.mpd(?:\?|$)|\.mp4(?:\?|$)|/hls/|/manifest", re.I)

def choose_proxy_for_url(url: str) -> Optional[str>:
    """
    Auto-proxy: prefer GUI 'proxy_url' if set; otherwise env vars.
    """
    # GUI setting first
    try:
        p = settings.get('proxy_url')
        if p:
            return p
    except Exception:
        pass
    # Env fallback
    for key in ("APP_DL_PROXY", "HTTP_PROXY", "http_proxy", "HTTPS_PROXY", "https_proxy"):
        val = os.environ.get(key)
        if val:
            return val
    return None

def resolve_media_with_selenium(url: str,
                                headless: bool = True,
                                use_undetected: bool = True,
                                proxy: Optional[str] = None,
                                user_agent: Optional[str] = None,
                                wait_seconds: int = 8,
                                click_play_selector: Optional[str] = None) -> Tuple[List[Dict], List[dict]]:
    """
    Returns (candidates, cookies). Each candidate is a dict with url and maybe headers.
    Uses selenium-wire when available to capture requests. Falls back to uc without wire (less visibility).
    """
    _send_gui("update_download_status", {"url": url, "status": "Resolving via Selenium..."})
    if not _SELENIUM_WIRE_OK and not _UC_OK:
        _send_gui("update_download_status", {"url": url, "status": "Selenium/uc not installed"})
        return [], []
    ...
    cookies: List[dict] = []
    try:
        if use_undetected and _UC_OK:
            # Try uc first; if selenium-wire is available, prefer selenium-wire for request capture
            if _SELENIUM_WIRE_OK:
                driver = _sw_webdriver.Chrome(options=chrome_opts, seleniumwire_options=seleniumwire_options)  # wire + std
                _send_gui("update_download_status", {"url": url, "status": "Browser started (selenium-wire)"})
            else:
                driver = uc.Chrome(options=chrome_opts, headless=headless)
                _send_gui("update_download_status", {"url": url, "status": "Browser started (undetected-chromedriver)"})
        else:
            if not _SELENIUM_WIRE_OK:
                _send_gui("update_download_status", {"url": url, "status": "selenium-wire missing"})
                return [], []
            driver = _sw_webdriver.Chrome(options=chrome_opts, seleniumwire_options=seleniumwire_options)
            _send_gui("update_download_status", {"url": url, "status": "Browser started (selenium-wire)"})
        driver.get(url)
        # give time for challenges and player init
        for i in range(max(1, wait_seconds)):
            time.sleep(1)
            _send_gui("update_download_status", {"url": url, "progress": f"waiting JS {i+1}/{wait_seconds}s"})

        if click_play_selector:
            try:
                el = driver.find_element("css selector", click_play_selector)
                el.click()
                time.sleep(2)
            except Exception:
                pass

        # Collect requests (selenium-wire)
        if hasattr(driver, "requests"):
            ...
            for req in getattr(driver, "requests", []):
                ...
                if u in seen:
                    continue
                if _MEDIA_PAT.search(u):
                    seen.add(u)
                    candidates.append({"url": u})
        else:
            # If we used uc without wire, use a heuristic: current_url if direct manifest
            cur = driver.current_url
            if _MEDIA_PAT.search(cur):
                candidates.append({"url": cur})
        ...
        try:
            logging.getLogger(__name__).exception("Selenium resolve error: %s", e)
        except Exception:
            pass
        _send_gui("update_download_status", {"url": url, "status": f"Selenium error: {e}"})
        return [], []
    finally:
        try:
            if driver:
...
def _write_temp_cookies(cookies: List[dict]) -> Optional[str]:
    if not cookies:
        return None
    txt = None
    # Prefer project's CookieManager if available
    cm = globals().get("CookieManager")
    if cm and hasattr(cm, "convert_to_netscape"):
        try:
            txt = cm.convert_to_netscape(cookies)
        except Exception:
            txt = _cookies_to_netscape(cookies)
    else:
        txt = _cookies_to_netscape(cookies)
    fd, path = tempfile.mkstemp(prefix="selenium_cookies_", suffix=".txt")
    with os.fdopen(fd, "w", encoding="utf-8") as f:
        f.write(txt)
    return path

def download_via_ffmpeg(input_url: str,
                        headers: Optional[Dict[str, str]] = None,
                        cookies_file: Optional[str] = None,
                        output_path: Optional[str] = None,
                        gui_id: Optional[str] = None) -> Tuple[bool, Optional[str]]:
    """
    Try a direct ffmpeg copy of HLS/DASH. Returns (ok, output_path).
    """
    ffmpeg_bin = shutil.which("ffmpeg")
    if not ffmpeg_bin:
        _send_gui("update_download_status", {"url": gui_id or input_url, "status": "ffmpeg not found"})
        return False, None
    if not output_path:
        base = f"media_{int(time.time())}.mp4"
        output_path = os.path.join(tempfile.gettempdir(), base)

    header_args = []
    if headers:
        # ffmpeg expects a merged headers string for -headers
        header_lines = [f"{k}: {v}" for k, v in headers.items() if v]
        if cookies_file:
            # If we have a cookie file, it's better to add Cookie header manually (optional)
            pass
        if header_lines:
            header_args = ["-headers", "\r\n".join(header_lines)]
    ...
    if header_args:
        cmd += header_args
    if cookies_file:
        # ffmpeg itself can't read netscape cookie files directly; yt-dlp is preferred for cookies.
        # We'll rely mostly on yt-dlp for cookies; keep this path minimal.
        pass
    cmd += ["-i", input_url, "-c", "copy", "-bsf:a", "aac_adtstoasc", output_path]

    _send_gui("update_download_status", {"url": gui_id or input_url, "status": "Downloading (ffmpeg)"})
    proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding="utf-8", errors="replace")
    try:
        for line in proc.stdout:
            if "speed=" in line or "time=" in line:
                _send_gui("update_download_status", {"url": gui_id or input_url, "progress": line.strip()})
        proc.wait()
        if proc.returncode == 0 and os.path.exists(output_path):
            _send_gui("add_completed", {
                "url": gui_id or input_url,
                "filetype": "video",
                "filename": os.path.basename(output_path),
                "path": output_path,
                "is_playlist": False,
                "timestamp": time.time(),
                "filesize_bytes": os.path.getsize(output_path)
            })
            return True, output_path
        else:
            _send_gui("update_download_status", {"url": gui_id or input_url, "status": "ffmpeg failed"})
            return False, None
    finally:
        try:
            proc.kill()
...
def _run_yt_dlp(url: str,
                cookies_file: Optional[str] = None,
                referer: Optional[str] = None,
                user_agent: Optional[str] = None,
                proxy: Optional[str] = None,
                gui_id: Optional[str] = None,
                outtmpl: Optional[str] = None) -> Tuple[bool, Optional[str]]:
    """
    Invoke yt-dlp with optional cookies/headers/proxy and stream progress back to GUI.
    """
    ytdlp_bin = shutil.which("yt-dlp") or shutil.which("yt_dlp") or "yt-dlp"
    args = [ytdlp_bin, "-f", "best", "--no-color", "--newline"]
    if outtmpl:
        args += ["-o", outtmpl]
    if cookies_file:
        args += ["--cookies", cookies_file]
    if user_agent:
        args += ["--add-header", f"User-Agent:{user_agent}"]
    if referer:
    ...
    _send_gui("update_download_status", {"url": gui_id or url, "status": "Downloading (yt-dlp)"})
    ...
    try:
        for line in proc.stdout:
            s = line.strip()
            if s:
                # Heuristic parse for progress and destination lines
                if s.startswith("[download] Destination:"):
                    final_path = s.split("Destination:",1)[1].strip()
                    _send_gui("update_download_status", {"url": gui_id or url, "filename": os.path.basename(final_path)})
                elif s.startswith("[download]") and "%" in s:
                    _send_gui("update_download_status", {"url": gui_id or url, "progress": s})
        proc.wait()
        ...
        if ok and final_path and os.path.exists(final_path):
            _send_gui("add_completed", {
                ...
                "is_playlist": False,
                "timestamp": time.time(),
                "filesize_bytes": os.path.getsize(final_path)
            })
...
def start_selenium_job(page_url: str,
                       gui_url_id: Optional[str] = None,
                       referer: Optional[str] = None,
                       user_agent: Optional[str] = None,
    ...
    """
    Resolve with Selenium, try ffmpeg (copy) if desired, else fallback to yt-dlp.
    Reports progress to GUI queue if available.
    """
    proxy = choose_proxy_for_url(page_url)
    _send_gui("update_download_status", {"url": gui_url_id or page_url, "status": "Starting Selenium resolver"})
    candidates, cookies = resolve_media_with_selenium(page_url,
                                                     headless=headless,
                                                     use_undetected=True,
                                                     proxy=proxy,
                                                     ...
                                                     wait_seconds=10)
    if not candidates:
        _send_gui("update_download_status", {"url": gui_url_id or page_url, "status": "No media found with Selenium"})
        return

    # choose first candidate
    media_url = candidates[0].get("url")
    if not media_url:
        _send_gui("update_download_status", {"url": gui_url_id or page_url, "status": "No usable candidate URL"})
        return

    cookies_file = _write_temp_cookies(cookies) if cookies else None

    # Prefer yt-dlp for cookies/HLS unless ffmpeg is explicitly requested first
    if try_ffmpeg_first:
        ok, path = download_via_ffmpeg(media_url, headers={"Referer": referer or page_url, "User-Agent": user_agent or ""},
                                       cookies_file=cookies_file, output_path=None, gui_id=gui_url_id or page_url)
        if ok:
            return

    # Fallback to yt-dlp
    _run_yt_dlp(media_url, cookies_file=cookies_file, referer=referer or page_url,
                user_agent=user_agent, proxy=proxy, gui_id=gui_url_id or page_url, outtmpl=outtmpl)

# ---- Flask endpoints (additive, minimal-risk) ----
try:
...
if _FLASK_OK:
    @app.route("/selenium/resolve", methods=["POST"])
    def selenium_resolve_endpoint():
        ...
        proxy = data.get("proxy") or choose_proxy_for_url(url)
        cands, cookies = resolve_media_with_selenium(url, headless=headless, use_undetected=True,
                                                     proxy=proxy, user_agent=ua, wait_seconds=int(data.get("wait", 8)))
        return jsonify({"candidates": cands, "cookies": cookies, "proxy_used": proxy}), (200 if cands else 404)

    @app.route("/selenium/download", methods=["POST"])
    def selenium_download_endpoint():
        data = request.get_json(force=True, silent=True) or {}
        url = data.get("url")
        if not url:
            return jsonify({"error": "url is required"}), 400
        gui_id = data.get("gui_url_id") or url
        referer = data.get("referer")
        ua = data.get("user_agent")
        headless = bool(data.get("headless", True))
        outtmpl = data.get("outtmpl")
        try_ffmpeg = bool(data.get("try_ffmpeg_first", True))

        t = threading.Thread(target=start_selenium_job, kwargs=dict(
            page_url=url,
            gui_url_id=gui_id,
            referer=referer,
            user_agent=ua,
            try_ffmpeg_first=try_ffmpeg,
            headless=headless,
            outtmpl=outtmpl,
        ), daemon=True)
        t.start()
        return jsonify({"status": "started", "gui_url_id": gui_id}), 202
# ================== End Selenium/Proxy/HLS Integration block ==================
...
# A simple dictionary to store download info
downloads_db = {}
...
def run_yt_dlp_command(args, cookies=None, timeout=None, platform_config=None, retry_count=0):
    ...
    try:
        ...
        try:
            _px = choose_proxy_for_url(platform_config.get("original_url", "")) if platform_config else choose_proxy_for_url("")
            if _px:
                base_args.extend(["--proxy", _px])
        except Exception:
            pass
...
def analyze_url():
    ...
    if analysis["needs_cookies"]:
        analysis["suggestions"].append(f"This {platform} content requires authentication. Pla
    ...
    platform_config['original_url'] = url  # Add platform info for cookie filtering
...
def get_error_suggestions(platform, stderr):
    ...
    platform_config = URLAnalyzer.get_platform_config(url)
    platform_config['platform'] = platform
    platform_config['original_url'] = url
...
class MainWindow(QMainWindow):
    ...
    def create_download_settings_panel(self):
        ...
        # --- Selenium Domains (one per line) ---
        sel_frame = QFrame()
        sel_frame.setLayout(QHBoxLayout())
        sel_frame.layout().setContentsMargins(0, 0, 0, 0)

        sel_label = QLabel("Selenium Domains:")
        sel_label.setFixedWidth(120)
        sel_frame.layout().addWidget(sel_label)

        self.selenium_domains_edit = QPlainTextEdit()
        self.selenium_domains_edit.setPlaceholderText("example.com\ncloudburst82.xyz")
        _sd = settings.get('selenium_domains') or []
        self.selenium_domains_edit.setPlainText("\n".join(_sd))
        self.selenium_domains_edit.textChanged.connect(lambda: settings.set('selenium_domains', [ln.strip() for ln in self.selenium_domains_edit.toPlainText().splitlines() if ln.strip()]))
        sel_frame.layout().addWidget(self.selenium_domains_edit)
        panel.layout().addWidget(sel_frame)

        # --- Proxy URL ---
        proxy_frame = QFrame()
        proxy_frame.setLayout(QHBoxLayout())
        proxy_frame.layout().setContentsMargins(0,0,0,0)

        proxy_label = QLabel("Proxy URL:")
        proxy_label.setFixedWidth(120)
        proxy_frame.layout().addWidget(proxy_label)

        self.proxy_edit = QLineEdit()
        self.proxy_edit.setPlaceholderText("http://user:pass@host:port or socks5://127.0.0.1:1080")
        _pv = settings.get('proxy_url') or ""
        self.proxy_edit.setText(_pv)
        self.proxy_edit.textChanged.connect(lambda: settings.set('proxy_url', self.proxy_edit.text().strip()))
        proxy_frame.layout().addWidget(self.proxy_edit)
        panel.layout().addWidget(proxy_frame)
        panel.layout().addStretch(1)
